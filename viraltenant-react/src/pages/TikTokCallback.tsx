...
export const TikTokCallback = () => {
  ...
  useEffect(() => {
    if (errorParam) {
      setStatus('error')
      setError(errorDescription || 'TikTok Autorisierung abgelehnt')
      return
    }
  ...
  const exchangeCodeForToken = async () => {
    ...
    try {
      ...
      const response = await fetch(`${awsConfig.api.user}/tiktok/oauth/callback`, {
        method: 'POST',
        headers: {
      ...
      setUserName(data.displayName || 'TikTok User')
      setStatus('success')
      ...
      if (window.opener) {
        window.opener.postMessage({ type: 'channel-updated', platform: 'tiktok' }, '*')
      }
      ...
      <div className="bg-dark-800 rounded-2xl p-8 max-w-md w-full shadow-xl border border-dark-700 text-center">
        ...
        {status === 'success' && (
          <>
            ...
            <h2 className="text-xl font-semibold mb-2 text-green-400">TikTok verbunden!</h2>
