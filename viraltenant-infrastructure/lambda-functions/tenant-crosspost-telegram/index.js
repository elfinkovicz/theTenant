...
async function postToTelegram(tenantId, post, settings) {
  ...
  if (post.imageKeys && post.imageKeys.length > 0) {
    ...
    console.log('Telegram: Resolved', post.imageKeys.length, 'images from imageKeys via CloudFront');
  } else if (post.imageUrls && post.imageUrls.length > 0) {
    imageUrls.push(...post.imageUrls);
    console.log('Telegram: Using', post.imageUrls.length, 'pre-resolved imageUrls');
