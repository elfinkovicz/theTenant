...
const getDefaultData = (tenantId) => ({
  tenant_id: tenantId,
  channels: [],
  settings: { maxChannels: 10, defaultVisibility: 'public' },
...
const enrichWithUrls = (data) => {
  ...
  if (data.channels) {
    data.channels = data.channels.map(channel => ({
      ...channel,
      iconUrl: channel.iconKey ? `https://${cfDomain}/${channel.iconKey}` : channel.iconUrl,
      bannerUrl: channel.bannerKey ? `https://${cfDomain}/${channel.bannerKey}` : channel.bannerUrl
    }));
  }
  return data;
};

async function getChannels(tenantId) {
  try {
    const result = await dynamodb.send(new GetCommand({
      TableName: process.env.TENANT_CHANNELS_TABLE, Key: { tenant_id: tenantId }
    }));
    return enrichWithUrls(result.Item || getDefaultData(tenantId));
  } catch (error) { return getDefaultData(tenantId); }
}

async function updateChannels(tenantId, updates) {
  const existing = await getChannels(tenantId);
  const item = { ...existing, ...updates, tenant_id: tenantId, updated_at: new Date().toISOString() };
  await dynamodb.send(new PutCommand({ TableName: process.env.TENANT_CHANNELS_TABLE, Item: item }));
  return enrichWithUrls(item);
}

async function generateUploadUrl(tenantId, fileName, fileType, uploadType) {
  const ext = fileName.split('.').pop();
  const key = `tenants/${tenantId}/channels/${uploadType}-${Date.now()}.${ext}`;
...
exports.handler = async (event) => {
  ...
  try {
    if (httpMethod === 'GET' && !path.includes('/upload-url')) {
      const data = await getChannels(tenantId);
    ...
    if (httpMethod === 'PUT' && !path.includes('/upload-url')) {
      ...
      const updated = await updateChannels(tenantId, JSON.parse(event.body || '{}'));
    ...
    if (httpMethod === 'POST' && path.includes('/upload-url')) {
      ...
      return { statusCode: 200, headers: corsHeaders, body: JSON.stringify(await generateUploadUrl(tenantId, fileName, fileType, uploadType || 'channel')) };
    }
