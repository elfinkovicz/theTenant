...
async function getWhatsAppSettings(tenantId) {
  try {
    const result = await dynamodb.send(new GetCommand({
      TableName: process.env.WHATSAPP_SETTINGS_TABLE,
...
async function updateWhatsAppSettings(tenantId, settings) {
  const existing = await getWhatsAppSettings(tenantId);
  const item = { 
    ...existing, 
    ...settings, 
    tenant_id: tenantId, 
...
exports.handler = async (event) => {
  ...
  // Get tenant ID from authorizer (required for /whatsapp/settings endpoint)
  ...
  try {
    ...
    if (httpMethod === 'PUT') {
      ...
      const updated = await updateWhatsAppSettings(tenantId, settings);
    ...
    if (httpMethod === 'POST') {
      ...
      if (!settings.enabled || !settings.phoneNumberId || !settings.accessToken) {
        return { 
          statusCode: 400, 
          headers: corsHeaders, 
          body: JSON.stringify({ message: 'WhatsApp nicht konfiguriert' }) 
        };
      }
      
      // Here you would implement actual WhatsApp API call
      // For now, just return success
      return { 
        statusCode: 200, 
        headers: corsHeaders, 
        body: JSON.stringify({ 
          message: 'Test-Nachricht würde gesendet werden',
          resolvedTenantId: tenantId
        })
      };
    }
