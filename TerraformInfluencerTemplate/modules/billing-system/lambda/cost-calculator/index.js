...
// Get next invoice number with AWS Account ID prefix
async function getNextInvoiceNumber() {
  ...
  const result = await docClient.send(
    new UpdateCommand({
      TableName: process.env.INVOICE_COUNTER_TABLE_NAME,
      Key: { counterId: 'invoice' },
  ...
  // Format: ACCOUNTID-YYYYMM-NNNNNN
  const yearMonth = new Date().toISOString().slice(0, 7).replace('-', '');
  return `${accountId}-${yearMonth}-${paddedCounter}`;
}
    ...
    if (!email || !email.includes('@')) {
      ...
      try {
        ...
        if (!email) {
          ...
          email = `${userId}@placeholder.com`; // Fallback
        }
      } catch (error) {
