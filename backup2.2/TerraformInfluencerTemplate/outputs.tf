...
output "billing_api_endpoint" {
  description = "Billing API Endpoint"
  value       = var.enable_billing_system ? "${module.user_auth[0].api_endpoint}/billing" : null
}

output "billing_webhook_url" {
  description = "Stripe Webhook URL (configure in Stripe Dashboard)"
  value       = var.enable_billing_system ? "${module.user_auth[0].api_endpoint}/billing/webhook" : null
}

output "billing_table_name" {
  description = "Billing DynamoDB Table"
  value       = var.enable_billing_system ? module.billing_system[0].billing_table_name : null
}
