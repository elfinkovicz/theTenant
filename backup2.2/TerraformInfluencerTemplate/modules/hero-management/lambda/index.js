...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === 'PUT' && path === '/hero') {
      ...
      // Get existing data first to preserve fields not being updated
      const existingResult = await docClient.send(new GetCommand({
        TableName: HERO_TABLE,
        Key: { heroId: HERO_ID }
      }));

      const existingHero = existingResult.Item || {};

      const hero = {
        heroId: HERO_ID,
        logoKey: body.logoKey !== undefined ? body.logoKey : existingHero.logoKey || null,
        title: body.title !== undefined ? body.title : existingHero.title || 'Your Brand',
        subtitle: body.subtitle !== undefined ? body.subtitle : existingHero.subtitle || '',
        logoSize: body.logoSize !== undefined ? body.logoSize : existingHero.logoSize || 160,
        navbarLogoKey: body.navbarLogoKey !== undefined ? body.navbarLogoKey : existingHero.navbarLogoKey || null,
        navbarTitle: body.navbarTitle !== undefined ? body.navbarTitle : existingHero.navbarTitle || 'Your Brand',
        // Hero section settings
        heroHeight: body.heroHeight !== undefined ? body.heroHeight : existingHero.heroHeight || 70,
        heroWidth: body.heroWidth !== undefined ? body.heroWidth : existingHero.heroWidth || 'full',
        heroBackground: body.heroBackground !== undefined ? body.heroBackground : existingHero.heroBackground || {
          type: 'gradient',
          value: 'linear-gradient(to bottom right, rgba(245, 158, 11, 0.2), rgba(3, 7, 18, 1))'
        },
        // Theme data
        themeId: body.themeId !== undefined ? body.themeId : existingHero.themeId || 'default',
        themeName: body.themeName !== undefined ? body.themeName : existingHero.themeName || 'Honigwabe (Standard)',
        themeColors: body.themeColors !== undefined ? body.themeColors : existingHero.themeColors || {
          primary: '#f59e0b',
          primaryHover: '#d97706',
          secondary: '#1f2937',
          secondaryHover: '#374151',
        ...
        // Design settings
        designSettings: body.designSettings !== undefined ? body.designSettings : existingHero.designSettings || {
          buttonSize: 'medium',
          buttonRoundness: 8,
          fontFamily: 'system-ui, -apple-system, sans-serif',
          fontSize: 16,
        ...
        // Stream settings
        streamTitle: body.streamTitle !== undefined ? body.streamTitle : existingHero.streamTitle || 'Live Stream',
        streamDescription: body.streamDescription !== undefined ? body.streamDescription : existingHero.streamDescription || 'Welcome to the stream!',
        updatedAt: new Date().toISOString()
      };
