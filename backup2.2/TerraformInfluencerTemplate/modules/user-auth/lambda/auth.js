...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Route handling - Exakte Matches für eindeutige Routen
    if (path === '/confirm-forgot-password' && method === 'POST') {
      return await handleConfirmForgotPassword(event);
    ...
    } else if (path === '/confirm' && method === 'POST') {
      return await handleConfirmSignUp(event);
    } else if (path === '/resend-code' && method === 'POST') {
...
async function handleForgotPassword(event) {
  try {
    ...
    let errorMessage = 'Fehler beim Senden des Reset-Codes';
...
async function handleConfirmForgotPassword(event) {
  console.log('ConfirmForgotPassword called');
  
  try {
    const body = JSON.parse(event.body);
    const { email, code, newPassword } = body;

    console.log('Request data:', { email, code: code ? '***' : undefined, newPassword: newPassword ? '***' : undefined });
    ...
    if (!email || !code || !newPassword) {
      ...
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Email, code und newPassword sind erforderlich' })
      };
    }
